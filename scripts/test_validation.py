...
def convert_part_features_to_api_format(part_features: Dict[str, str]) -> Dict[str, str]:
...
def make_prediction_request(part_characteristics: Dict[str, str], max_processes: int = 10) -> Tuple[bool, List[List[str]], str]:
...
def test_single_entry(entry: Dict[str, Any], entry_index: int) -> Dict[str, Any]:
    ...
    success, predicted_chains, error_msg = make_prediction_request(
        api_features, 
        max_processes=len(expected_chains)
    )
    ...
    print(f"  Expected {len(expected_chains)} chains, got {len(predicted_chains)} chains")
    ...
    for i, expected_chain in enumerate(expected_chains):
        ...
        for j, predicted_chain in enumerate(predicted_chains):
            if compare_process_chains(expected_chain, predicted_chain):
                found_match = True
                print(f"  ✅ Expected chain {i+1} matches predicted chain {j+1}: {expected_chain}")
                break
...
def run_validation_tests(json_file_path: str = "data/parts_and_process_chains.json") -> Dict[str, Any]:
    ...
    print(f"Entries with ALL process chains correct: {entries_with_all_correct}/{total_entries} ({100*entries_with_all_correct/total_entries:.1f}%)")
    print(f"Individual process chains correct: {correct_individual_chains}/{total_individual_chains} ({100*correct_individual_chains/total_individual_chains:.1f}%)")
    print()
    ...
    incorrect_entries = [r for r in results if r["api_success"] and not r["all_chains_correct"]]
    if incorrect_entries:
        print(f"ENTRIES WITH INCORRECT PREDICTIONS ({len(incorrect_entries)}):")
        for result in incorrect_entries:
            print(f"  Entry {result['entry_index'] + 1}: {result['correct_chain_count']}/{result['total_chain_count']} correct")
