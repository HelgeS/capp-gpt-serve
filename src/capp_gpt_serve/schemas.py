...
class InferenceRequest(BaseModel):
    ...
    max_processes: Optional[int] = Field(default=10, ge=1, le=20, description="Maximum number of processes to return")
    temperature: Optional[float] = Field(default=1.0, ge=0.1, le=2.0, description="Sampling temperature")
    include_confidence: Optional[bool] = Field(default=True, description="Include confidence scores in response")


class InferenceResponse(BaseModel):
    """Response model for manufacturing process inference."""
    manufacturing_processes: List[str] = Field(..., description="Recommended manufacturing processes")
    confidence_scores: Optional[List[float]] = Field(None, description="Confidence scores for each process")
...
class TokenCategoriesResponse(BaseModel):
    ...
    manufacturing_processes: List[str] = Field(..., description="Possible manufacturing processes")
