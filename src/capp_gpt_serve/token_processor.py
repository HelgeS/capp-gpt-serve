...
class TokenProcessor:
    ...
    def json_to_sequence(self, input_data: Dict[str, Any]) -> torch.Tensor:
        ...
        try:
            ...
            for key in ["geometry", "holes", "external_threads", "surface_finish", "tolerance", "batch_size"]:
    ...
    def sequence_to_json(self, sequence: torch.Tensor, include_confidence: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("No <EOC> token found, treating entire sequence as processes")
            ...
            for token_id in process_tokens:
                ...
                if token_str in self.id2token:
                    ...
                    if not token_name.startswith("<") and token_name not in [
                        "geometry_pure_axisymmetric", "geometry_axisymmetric_with_prismatic_features",
                        "geometry_prismatic", "geometry_prismatic_with_axisymmetric_features",
                        "geometry_prismatic_with_freeform_features", "geometry_freeform", "geometry_unconventional",
                        "holes_none", "holes_normal", "holes_normal_threaded", "holes_normal_functional",
                        "holes_large", "holes_large_threaded", "holes_large_functional",
                        "external_threads_yes", "external_threads_no",
                        "surface_finish_rough", "surface_finish_normal", "surface_finish_good", "surface_finish_very_good",
                        "tolerance_rough", "tolerance_normal", "tolerance_medium", "tolerance_tight",
                        "batch_size_prototype", "batch_size_small", "batch_size_medium", "batch_size_large", "batch_size_mass"
                    ]:
    ...
    def get_valid_tokens(self) -> Dict[str, List[str]]:
        ...
        for token in self.token2id.keys():
            ...
            elif not token.startswith("<") and token not in sum(categories.values(), []):
