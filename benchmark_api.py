...
class APIBenchmark:
    ...
    async def make_request(self, session: aiohttp.ClientSession, endpoint: str, 
                          method: str = "GET", data: Dict = None) -> BenchmarkResult:
            ...
            if method.upper() == "POST":
                async with session.post(url, json=data) as response:
                    ...
                    return BenchmarkResult(
                        ...
                        error_message="" if response.status < 400 else f"HTTP {response.status}"
                    )
            else:
    ...
    async def benchmark_sequential(self, session: aiohttp.ClientSession, 
                                 endpoint: str, requests: int, method: str = "GET", 
                                 data: Dict = None) -> BenchmarkStats:
    ...
    async def benchmark_concurrent(self, session: aiohttp.ClientSession, 
                                 endpoint: str, requests: int, concurrency: int,
                                 method: str = "GET", data: Dict = None) -> BenchmarkStats:
        """Run concurrent benchmark with specified concurrency level."""
        print(f"🚀 Running concurrent benchmark: {requests} requests, {concurrency} concurrent")
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if completed % max(1, requests // 10) == 0:
                print(f"  Progress: {completed}/{requests} ({(completed/requests)*100:.1f}%)")
        
        total_time = time.time() - start_time
        
        return self._calculate_stats(results, total_time)
    
    async def benchmark_sustained(self, session: aiohttp.ClientSession, 
                                endpoint: str, duration_seconds: int, concurrency: int,
                                method: str = "GET", data: Dict = None) -> BenchmarkStats:
        """Run sustained load test for specified duration."""
        print(f"🚀 Running sustained benchmark: {duration_seconds}s duration, {concurrency} concurrent")
        ...
        while time.time() < end_time:
            ...
            if tasks:
                ...
                if request_count % (concurrency * 5) == 0:
                    ...
                    print(f"  Progress: {elapsed:.1f}s elapsed, {request_count} requests, {rps:.1f} RPS")
...
async def main():
    ...
    parser.add_argument("--endpoint", choices=["health", "tokens", "predict", "all"], 
                       default="all", help="Which endpoint to benchmark (default: all)")
    parser.add_argument("--test-type", choices=["sequential", "concurrent", "sustained", "all"], 
                       default="all", help="Which test type to run (default: all)")
    ...
    async with aiohttp.ClientSession(timeout=benchmark.timeout) as session:
        ...
        for endpoint, method, data in endpoints:
            ...
            if args.test_type in ["sequential", "all"]:
                ...
                benchmark.print_stats(stats, f"Sequential Test - {endpoint_name.title()} Endpoint")
            ...
            if args.test_type in ["concurrent", "all"]:
                ...
                benchmark.print_stats(stats, f"Concurrent Test - {endpoint_name.title()} Endpoint")
            ...
            if args.test_type in ["sustained", "all"]:
                ...
                benchmark.print_stats(stats, f"Sustained Test - {endpoint_name.title()} Endpoint")
